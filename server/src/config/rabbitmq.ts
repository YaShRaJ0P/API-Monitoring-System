...
export class RabbitMQ {
     ...
     */
    static async connect(url: string, retryCount = 0): Promise<Channel> {
        if (this.channel) {
            return this.channel;
        }
        ...
        this.connectionPromise = (async () => {
            try {
                ...
                if (retryCount < 10) {
                    log.error(`Connection failed, retrying in ${delay}ms...`, { attempt: retryCount + 1 });
                    await new Promise(resolve => setTimeout(resolve, delay));
                    return this.connect(url, retryCount + 1);
                }
